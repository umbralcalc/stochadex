...
// TestModelIndexUpToDate fails if the committed domain model index artifacts
...
func TestModelIndexUpToDate(t *testing.T) {
	...
	if changed {
		t.Error("stale domain model index — run `go generate ./cmd/model-index` to regenerate")
	}
}
