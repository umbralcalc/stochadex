...
recurrence once several stubs exist. The **cross-model index** ([`INDEX.md`](INDEX.md),
...
`go generate ./cmd/model-index` (the cross-model index, which auto-discovers the new stub). The `behaviour_test.go` expected-behaviour
