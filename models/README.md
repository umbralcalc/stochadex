...
| [limit-order-book](limit-order-book/card.md) | Limit-order-book microsimulation: a shared latent-activity driver → depth-damped stochastic order flow (arrivals, churn, cancellations, marketable orders) → a deterministically matched book → spread, depth and the arrival–depth stability coupling. Purely declarative downstream — the catalogue's first entry whose bespoke Go is a re-expression of the data, not the reverse | [repo](https://github.com/umbralcalc/cryptobook) |
| [solar-fleet](solar-fleet/card.md) | Aggregate solar-PV fleet output: deterministic solar geometry (a replayed clear-sky irradiance driver) → a distance-coupled full-covariance clear-sky-index field (one Cholesky-correlated OU innovation per step) → summed fleet generation, whose flagship is dispersion-smoothing — spreading sites apart lowers aggregate variability at fixed capacity. Born-declarative: the downstream forward model is pure config | [repo](https://github.com/umbralcalc/solar-fleet) |
