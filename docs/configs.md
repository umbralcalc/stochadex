...
The [Quickstart](quickstart.html) builds a simulation in Go. You can also describe an entire
run — the model, the analysis, the inference — in **one YAML file** and execute it with a
prebuilt binary. A config that names no Go anywhere resolves and runs **in-process**: no code
generation, no `go run`, no Go toolchain on the machine.
